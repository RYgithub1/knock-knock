...
- ユーザー管理
  ...
  - 文字数制約や同名禁止の制約。
  - 右方の四角内に、ログイン中の場合は名前、非ログイン中の場合は visiter と表示(在地情報を取得済みの場合現在地も記載)。
...
- プロフィール(about)
  ...
  - プロフィールページの下方に、ロケーション取得ボタンを配置。1 クリックで位置情報を保存しマップ等へ反映。
  - 他の人のプロフィール閲覧画面を参照している場合、下方にお気に入り(Door Hanger)機能を実装。非同期対応。
...
- 活動内容の一覧表示(DoorS)
  ...
  - 表に最終メッセージや、最終更新日を表示。
  - メッセージを行うと、お気に入り(door hangers)での表示が消える(重複排除)。
