...
- トップ画面
  ...
  - スライドショーで世界遺産を表示
  - Adventure(フッター)にて、旅関連の格言を閲覧可能。kaminari 使用してページネーション機能を実装。seed で初期値を埋め込む設計。
  - Our Team(フッター)をクリックするとモーダルでポリシーを表示。モーダルの周辺をクリックするとトップ画面に戻る。
  - contacts(フッター)から、問い合わせの受付。

- マップ

  - API を活用。
  - 旅人の位置を一覧表示（初期は自分の位置がセンター表示）。
  - 個々人の情報は、カーソルを合わせると非同期で表示。
  - 情報ウィンドウ内のリンクにより気になった人の、プロフィール閲覧へアクセス可能。
...
- ユーザー管理
  ...
  - 文字数や同盟禁止の制約。
  - ログイン中の場合は名前、非ログイン中の場合は visiter と表示。

- プロフィール(about)

  - プロフィールの表示、新規作成編集。
  - 画像のアップロードは carrierwave を使用。
  - プレビュー機能。アイコンは 1 枚、写真は複数枚投稿可能。
  - 国籍(nationality)は、active hash を活用。
  - 言語(language)は、タグ付け機能を実装。acts_as_taggable と、jQuery プラグインの tag-it を使用。タグ名は自由に入力可能。
  - 下方に、ロケーション取得ボタンを配置。1 クリックで位置情報を保存しマップへ反映。
  - 他人のプロフィールを閲覧する画面の場合、下方にお気に入り(Door Hanger)機能を表示（非同期）。

- メッセージ(knock-knock door)

  - マップ → 他の人のプロフィール →door ノブチェック → 旅人間のメッセージルームを作成。
  - 重複した２名間でのルームは作成不可。
  - ライン風メッセージ画面。テキストと画像の送付が可能。
  - アクティビティ一覧から、メッセージルームの削除が可能。

- アクティビティ一覧表示(DoorS)

  - ハンガーをかけたドアや、メッセージを行ったドアを一覧表示。
  - bootstrap を使用。
  - 新規メッセージがある場合、赤文字で「New message」との通知が表示。
  - 最終メッセージや、最終更新日を表示。
  - メッセージを行うと、door hangers での表示が消える(重複なく表示)。
